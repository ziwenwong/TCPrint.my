...
const productCategories = [
  ...
  },
  {
    ...
    category: "Sticker Printing",
    subtitle: "Custom size large format sticker with square-centimeter pricing.",
    description: "Same sticker options as Label Sticker, with custom size only and RM0.002/sq cm estimate.",
    priceLabel: "RM0.002/sq cm",
    mode: "large-sticker",
...
function getStickerQuantity() {
  if (activeProduct().mode === "large-sticker") {
...
function calculateLargeFormatStickerPrice(product) {
  ...
  const quantity = getStickerQuantity();
  const unit = 0.002;
  const printing = width * height * unit * quantity;
  const postage = deliveryCost();
  const shape = selectedText(selectors.stickerShapeSelect);
  const material = selectedText(selectors.stickerMaterialSelect);
  const lamination = selectedText(selectors.stickerLaminationSelect);
  ...
  return {
    ...
    lamination,
    unit,
    unitLabel: `${formatMoneyThreeDecimals(unit)} / sq cm`,
    printing,
    postage,
    total: printing + postage,
    note: "Large format sticker is calculated at RM0.002/sq cm. Final price depends on artwork and finishing.",
  };
}
...
function showControls(mode) {
  selectors.stickerControls.hidden = !(mode === "sticker" || mode === "large-sticker");
...
function renderInteractiveProductPreview(result) {
  if (result.mode === "sticker" || result.mode === "large-sticker") {
...
function syncFormVisibility() {
  const product = activeProduct();
  const isLargeFormatSticker = product.mode === "large-sticker";
  ...
  selectors.stickerSizeField.hidden = isLargeFormatSticker;
  selectors.customSizeFields.hidden = !(isCustomStickerSize || isLargeFormatSticker);
  selectors.stickerQuantitySelect.hidden = isLargeFormatSticker;
  selectors.customStickerQuantity.hidden = !(isCustomStickerQuantity || isLargeFormatSticker);
  ...
  if (!isCustomStickerQuantity && !isLargeFormatSticker) {
...
function setActiveProduct(productId, shouldScroll = false) {
  ...
  if (product.mode === "large-sticker") selectors.customStickerQuantity.value = "1";
